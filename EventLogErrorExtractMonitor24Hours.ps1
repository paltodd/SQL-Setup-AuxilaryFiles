...
$name = -join("\\mk-ds02\MKC_Public\Information Technology\Database Admins\EventLogs\24Hours\",$env:computername, "ELApplication.csv");

#Export Error and Warning event details
get-eventlog -logname Application -After (Get-Date).AddHours(-24) | where {($_.ENtryType -eq 'Warning' -or $_.ENtryType -eq 'Error')} | export-csv $name
